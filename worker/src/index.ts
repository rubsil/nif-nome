...
function isUsablePublicName(value: unknown): value is string {
  ...
  const v = value.replace(/\s+/g, " ").trim(); if (v.length < 3 || v.length > 100) return false;
  const n = normaliseText(v);
  if (/^s\s+e\s+public/.test(n)) return false;
  if (/licenciada|líder|lider|mercado|informação para negócios|informacao para negocios|informa d&b|relatório grátis|relatorio gratis/.test(n)) return false;
  if (/^(nif|contribuinte|denominação|denominacao|morada|atividade|codigo postal|cnae|cae|privacy|cookies|login|register|search|next|previous)$/i.test(n)) return false;
...
async function discoverWithEmpresite(nif: string, legalName: string | null) { const found = await findByNifEmpresite(nif, legalName); if (!found) return null; const names = found.publicNames.filter(name => cleanPublicName(name)).map(name => ({ name: cleanPublicName(name)!, type: "nome comercial", confidence: 0.95, sources: [{ name: "Empresite", source_type: "directory", url: found.sourceUrl }] })); return { nif, legalName: cleanLegalName(found.legalName), publicName: names[0]?.name || null, publicNames: names, location: found.address || null, address: found.address, website: null, activity: null, cae: null, source: "Empresite", sourceUrl: found.sourceUrl }; }
async function discoverWithEInforma(nif: string) { const found = await findByEInforma(nif); if (!found) return null; const publicNames = found.publicNames.map(cleanPublicName).filter((name): name is string => !!name); return { nif, legalName: cleanLegalName(found.legalName), publicName: publicNames[0] || null, publicNames: publicNames.map(name => ({ name, type: "nome comercial", confidence: 0.82, sources: [{ name: "eInforma", source_type: "directory", url: found.sourceUrl }] })), location: found.address || null, address: found.address, website: found.website, activity: found.activity, cae: null, source: "eInforma", sourceUrl: found.sourceUrl }; }
async function discoverWithPublicacoes(nif: string) { const found = await findByPublicacoes(nif); if (!found) return null; const candidates = found.publicNames.map(cleanPublicName).filter((name): name is string => !!name); return { nif, legalName: cleanLegalName(found.legalName), publicName: candidates[0] || null, publicNames: candidates.map(name => ({ name, type: "nome público", confidence: 0.85, sources: [{ name: "Publicações do Ministério da Justiça", source_type: "government", url: found.sourceUrl }] })), location: found.address || null, address: found.address, website: null, activity: null, cae: null, source: "publicacoes.mj.pt", sourceUrl: found.sourceUrl }; }
...
function addNames(base: any, discovered: any) { const names = (discovered?.publicNames || []).filter((item: any) => cleanPublicName(item?.name)); const legal = cleanLegalName(base?.legalName); const filtered = names.filter((item: any) => !legal || normaliseText(item.name) !== normaliseText(legal)); return filtered.length ? { ...base, publicNames: filtered, publicName: filtered[0].name } : base; }
...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === "/api/nearby" && request.method === "GET") {
    ...
    if (address.length < 5) return json({ error: "Morada inválida." }, { status: 400 });
    try { return json(await findNearby(address)); } catch (error) { return json({ error: String(error).replace(/[\r\n]/g, " ").slice(0, 200), source: "OpenStreetMap" }, { status: 502 }); }
  }
  ...
  if (url.pathname === "/api/discover" && request.method === "GET") {
    ...
    const empresite = await run("Empresite", () => discoverWithEmpresite(nif, base?.legalName || null));
    if (empresite?.publicNames?.length) { base = addNames(base, empresite); return await finish(); }

    const einforma = await run("eInforma", () => discoverWithEInforma(nif));
    if (einforma?.publicNames?.length) { base = addNames(base, einforma); return await finish(); }

    const mj = await run("publicacoes.mj.pt", () => discoverWithPublicacoes(nif));
    if (mj?.publicNames?.length) { base = addNames(base, mj); return await finish(); }

    await run("rigorbiz", () => discoverWithRigorBiz(nif));
    // Deliberately no generic DuckDuckGo name guessing anymore. Search snippets
    // are useful for discovering source pages, but not reliable evidence of a
    // commercial name on their own.
    return await finish();
  }
