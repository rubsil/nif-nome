...
async function discoverWithEmpresite(nif: string, legalName: string | null) { const found = await findByEmpresite(nif, legalName); if (!found) return null; const names = found.publicNames.filter(name => cleanPublicName(name)).map(name => ({ name: cleanPublicName(name)!, type: "nome comercial", confidence: 0.95, sources: [{ name: "Empresite", source_type: "directory", url: found.sourceUrl }] })); return { nif, legalName: cleanLegalName(found.legalName), publicName: names[0]?.name || null, publicNames: names, location: found.address || null, address: found.address, website: null, activity: null, cae: null, source: "Empresite", sourceUrl: found.sourceUrl }; }
...
export default { async fetch(request: Request, env: Env): Promise<Response> {
  ...
  if (url.pathname === "/api/discover" && request.method === "GET") {
    ...
    await run("Empresite", () => discoverWithEmpresite(nif, base?.legalName || null));
