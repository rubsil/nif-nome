...
function extractCommercialNames(text: string, legalName: string | null): string[] {
  ...
  // Only accept explicit commercial/brand labels. Do not infer a name from page navigation.
  const patterns = [
...
function extractFromPage(text: string, nif: string, sourceUrl: string): EInformaResult | null {
  ...
  const legalName = cleanLegalName(field(text, ["Denominação", "Razão Social"], ["Designações anteriores", "Denominações anteriores", "Denominação Comercial", "Morada", "Atividade"]));
