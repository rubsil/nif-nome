...
async function fetchPage(url: string): Promise<string | null> {
  const response = await fetch(url, { headers: { "user-agent": "Mozilla/5.0 (compatible; nif-nome/1.1; public business lookup)", accept: "text/html,application/xhtml+xml" } });
...
function extractSearchUrls(html: string): string[] {
  ...
  // DuckDuckGo normally returns relative /l/?uddg=... result links.
  // The old code only searched for absolute URLs and could therefore miss
  // the correct Empresite page entirely.
...
export async function findByNif(nif: string, legalName: string | null): Promise<EmpresiteResult | null> {
  ...
  for (const term of terms) {
    ...
    const searchResponse = await fetch(searchUrl, { headers: { "user-agent": "Mozilla/5.0 (compatible; nif-nome/1.1; public business lookup)", accept: "text/html" } });
    if (!searchResponse.ok) continue;
    const urls = extractSearchUrls(await searchResponse.text()).slice(0, 12);
