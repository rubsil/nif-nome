...
function cleanStreetForGeocoding(value: string): string {
  return value
    ...
    .replace(/\b\d+[A-Za-z]?\b/g, "") // Remove número da porta para evitar falha no OSM
    .replace(/\s*,\s*/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
...
function locality(value: string): string {
  const match = value.match(/(?:\d{4}-\d{3})\s+(.+)$/i);
  if (!match) return "Horta";
  return match[1].replace(/\b(?:HRT|MAD|Portugal)\b/gi, "").replace(/\s+/g, " ").trim() || "Horta";
}
...
async function geocode(query: string): Promise<any | null> {
  ...
  const response = await fetch(url.toString(), { 
    headers: { 
      "user-agent": "nif-nome/2.6 (public business lookup bot)", 
...
export async function findNearby(address: string): Promise<{ places: NearbyPlace[]; geocoded: { display_name: string; lat: number; lon: number } | null; source: string }> {
  ...
  const streetClean = cleanStreetForGeocoding(raw);
  const loc = locality(raw);

  let geo: any | null = null;

  // Tentativas de geocodificação progressivas (da mais específica à mais genérica)
  const candidates = [
    pc ? `${pc}, Portugal` : "",
    streetClean && pc ? `${streetClean}, ${pc}, Portugal` : "",
    streetClean && loc ? `${streetClean}, ${loc}, Portugal` : "",
    `${loc}, Faial, Portugal`
  ...
  const radius = 400; // Raio ampliado ligeiramente para 400m
  ...
  for (const endpoint of endpoints) {
    try {
      const overpass = await fetch(endpoint, { 
        method: "POST", 
        headers: { "content-type": "text/plain; charset=utf-8", "user-agent": "nif-nome/2.6" }, 
        body: query 
      });
